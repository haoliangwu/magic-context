...
function getLastNudgeLevel(db: Database, sessionId: string) {
  return getChannel1NudgeState(db, sessionId).level;
}
function setLastNudgeLevel(db: Database, sessionId: string, value: "" | "gentle" | "firm" | "urgent") {
  const cur = getChannel1NudgeState(db, sessionId);
  setChannel1NudgeState(db, sessionId, { level: value, ordinal: cur.ordinal });
}
// v0.37 shouldTriggerChannel2 (1/3 usable, 10k floor) — preserves DSH behavior vs new 0.75 severity
function shouldTriggerChannel2(input: { reclaimableTokens: number; usableTokens: number }): boolean {
  if (input.reclaimableTokens < 10_000) return false;
  if (input.usableTokens <= 0) return true;
  return input.reclaimableTokens >= input.usableTokens * (1 / 3);
}
...
export function maybeNudgeChannels(
  ...
  try {
    ...
    if (reclaimable >= CHANNEL1_FLOOR_TOKENS) {
      ...
      const estimatedInputTokens = lastInputTokens + reclaimable;
      // ponytail: map DSH metrics to new hygiene baseline; keep old pressure gate (new core dropped it)
      let decision = decideChannel1({
        baselineU: reclaimable,
        baselineT: estimatedInputTokens,
        turnDeltaU: 0,
        turnDeltaT: 0,
        lastNudgeUndropped: getLastNudgeUndropped(db, sessionId),
        lastNudgeLevel: getLastNudgeLevel(db, sessionId) as any,
        hasRecentReduce: false,
      });
      if (pressure < 0.8) decision = { ...decision, fire: false };
      ...
      if (decision.fire) {
        ...
        const reminder = buildChannel1Reminder(decision.level, decision.undroppedTokens, workingWindowTokens, hint);
    ...
    if (shouldTriggerChannel2({ reclaimableTokens: reclaimable, usableTokens: usable })) {
      ...
      if (state === "") {
        const tags = getTagsBySession(db, sessionId);
        const hint = oldestReclaimableToolTags(tags, protectedTags);
        const reminder = buildChannel2Reminder(reclaimable, usable, hint);
